...
def get_extra_serializers():
    ...
    from planning import Candidate, SystemConfig as Sc
    return [
        get_np_serializer,
        Sc.__serializer__,
...
def get_log_file_handler(log_file):
    formatter = logging.Formatter \
        ('%(asctime)s:%(msecs)03d %(name)-27s %(levelname)-8s %(message)s',
         datefmt='%d-%m-%y %H:%M:%S')
...
class TimeSeries:
    ...
    *start* date and resolution *res* in seconds."""
    ...
    def get_item_by_index(self, index):
        date = self.start.shift(seconds=int(index * self.res))
        return date, self._data[index]
